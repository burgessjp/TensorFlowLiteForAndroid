 ...
 * @author _SOLID
 */
public class TFLiteImageClassifierFloat implements Classifier {
    ...
    private TFLiteImageClassifierFloat(TensorFlowClassifyConfig config) {
        this.config = config;
    }

    public static Classifier create(TensorFlowClassifyConfig config) {
        TFLiteImageClassifierFloat classifierFloat = new TFLiteImageClassifierFloat(config);
        ...
        if (config.getFileSource() == FileSource.ASSERT) {
            ...
            classifierFloat.imgData =
                    ByteBuffer.allocateDirect(
                            4 * DIM_BATCH_SIZE * config.getInputSizeX() * config.getInputSizeX() * DIM_PIXEL_SIZE);
            ...
            classifierFloat.labelProb = new float[1][classifierFloat.labels.size()];
        }
    ...
    private void putPixelData(int val) {
        imgData.putFloat((((val) & 0xFF) - 123));
        imgData.putFloat((((val >> 8) & 0xFF) - 117));
        imgData.putFloat((((val >> 16) & 0xFF) - 104));
    }
